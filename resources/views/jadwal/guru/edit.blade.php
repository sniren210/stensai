      ...
      <div class="row mb-2">
        ...
        <div class="col-sm-6">
          <ol class="breadcrumb float-sm-right">
            <li class="breadcrumb-item"><a href="#">Jadwal Guru</a></li>
      ...
      <div class="row">
        ...
        <div class="col-md-12">
          ...
          <div class="card card-primary">
            ...
            <!-- /.card-header -->
            <!-- form start -->
            <form method="POST" action="/jadwal-guru/{{$jadwal->id}}">
              @csrf
              @method('put')
              <div class="card-body">
                <div class="form-group">
                  <label>Guru</label>
                  <select class="custom-select @error('guru') is-invalid @enderror" name="guru">
                    @foreach ($guru as $data)                        
                    <option value="{{$data->id}}" {{($data->id == $jadwal->guru_id) ? 'selected' : ' '}}>{{$data->nama}} </option>
                    @endforeach
                  </select>
                  @error('guru')
              ...
              <div class="card-footer">
                <a href="{{ url('/jadwal-guru/'.$jadwal->guru_id) }}" type="submit" class="btn btn-link">Kembali</a>
